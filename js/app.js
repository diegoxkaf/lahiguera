...
let capasPorNombre = {};
let capasOrdenadas = [];
// Arreglo para mantener el orden de las capas
let capaBaseActual = null;
// Capa base actualmente seleccionada
...
function bindPopup(feature, layer, capaNombre, tema) {
    ...
    // Obtener el nombre personalizado de la capa
    const nombrePersonalizado = config.nombrePersonalizado || capaNombre;
...
function agregarEtiquetas(layer, capaConfig) {
    ...
    // Extraer estilos personalizables con valores por defecto
    const textColor = estilo.color || '#000000'; // Color del texto por defecto
    const fontSize = estilo.fontSize || '12px'; // Tamaño de la fuente por defecto
    const fontFamily = estilo.fontFamily || 'Arial, sans-serif'; // Familia de la fuente por defecto
    const bufferColor = estilo.bufferColor || '#FF0000'; // Color del contorno por defecto
    const bufferWidth = estilo.bufferWidth !== undefined ? estilo.bufferWidth : 1; // Ancho del contorno por defecto
    const offsetY = estilo.offsetY !== undefined ? estilo.offsetY : 0; // Vertical offset en píxeles
    ...
    layer.eachLayer(featureLayer => {
        ...
        if (feature.geometry.type === 'Point') {
            labelPosition = featureLayer.getLatLng();
             ...
             // Calcular el centro para líneas
            if (featureLayer.getBounds().isValid()) {
                 labelPosition = featureLayer.getBounds().getCenter();
            } else {
                console.warn('Bounds inválidos para la capa de línea, omitiendo etiqueta:', feature);
                return;
            }

        } else if (feature.geometry.type === 'Polygon' || feature.geometry.type === 'MultiPolygon') {
             // Usar getCenter() para polígonos si está disponible, de lo contrario usar el centro de los bounds
             if (typeof featureLayer.getCenter === 'function' && featureLayer.getBounds().isValid()) {
                 labelPosition = featureLayer.getCenter();
             } else if (featureLayer.getBounds().isValid()) {
                  labelPosition = featureLayer.getBounds().getCenter();
             } else {
                 console.warn('Bounds inválidos para la capa de polígono, omitiendo etiqueta:', feature);
                 return;
             }
        }
        ...
        // Crear el contenido HTML con estilos inline para el contorno del texto
        const labelContentHTML = `
            <span class="outlined-text" style="
                color: ${textColor};
                font-size: ${fontSize};
                font-family: ${fontFamily};
                -webkit-text-stroke: ${bufferWidth}px ${bufferColor};
                text-stroke: ${bufferWidth}px ${bufferColor};
            ">${labelText}</span>
        `;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const initialThemeButton = document.querySelector(`.theme-btn[data-theme="${temaInicial}"]`);
    if (initialThemeButton) {
        initialThemeButton.click();
    } else {
        console.error(`Botón para el tema inicial "${temaInicial}" no encontrado.`);
        // Cargar capas comunes si no se encuentra el tema inicial
        commonLayers.forEach(capaNombre => {
             cargarCapaIndividual(capaNombre);
             const checkbox = document.getElementById(`check-${capaNombre}`);
             if (checkbox) checkbox.checked = true;
         });
         // Si no hay un tema inicial, puedes pasar null o un objeto vacío a estas funciones si lo manejan
         actualizarLeyenda(); // Podría necesitar manejo para tema = null/undefined
         actualizarCapasSidebar(temaInicial); // Podría necesitar manejo para tema = null/undefined
    }
});
// Actualizar leyenda dinámica (Mantengo la función actualizada del paso anterior)
function actualizarLeyenda() {
    ...
    capasActivas.forEach(capaNombre => {
        ...
        Object.keys(temasConfig).forEach(tema => {
            if (temasConfig[tema].estilo &&
                temasConfig[tema].estilo[capaNombre]) {
                capaConfig = temasConfig[tema].estilo[capaNombre];
                temaEncontrado = tema;
            }
        });
...
// Actualizar checkbox event listeners para actualizar la leyenda (Mantengo la función actualizada del paso anterior)
function actualizarCapasSidebar(tema) {
    const layerList = document.getElementById('sidebar-layers');
    layerList.innerHTML = ''; // Limpiar la lista de capas
...
// Modificar el manejo de temas para actualizar la leyenda (Mantengo la función actualizada del paso anterior)
// Manejo de temas
document.querySelectorAll('.theme-btn').forEach(btn => {
    btn.addEventListener('click', function() {
        ...
        // Asegúrate de que commonLayers esté definido antes de usarse
        const capasDelTema = [...(typeof commonLayers !== 'undefined' ? commonLayers : []), ...(temasConfig[tema].capas || [])];


        // Cargar las capas desde cero
        capasDelTema.forEach(capaNombre => {
            // Verificar si la capa ya existe en capasPorNombre antes de cargarla individualmente
...
const commonLayers = ['limite_comunal_linea'];
